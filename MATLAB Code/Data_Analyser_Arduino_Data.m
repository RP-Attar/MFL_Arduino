...
% Columns are [N1, N2, N3, S1, S2, S3, L1, L2, L3, C1, C2, C3]
formatSpec = "%d";
maxColLength = 137;                         % found by looking at list of sizes
allData = zeros(maxColLength, 9);
allColLength = [];

for i = 1:length(fileNames)
    fileID = fopen(fileNames(i), "r");
    col = fscanf(fileID, formatSpec);
...
distance = linspace(0, 40, maxColLength);
...
averageData = zeros(maxColLength, 3);
...
xlabel('distance from first magnet (mm)')
ylabel('voltage (V)')
legend('no cut', 'small cut (~40%)', 'large cut (~60%)', 'corrosion');
